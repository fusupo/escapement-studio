  ...
  writeSummary(run: ExecutionRunRecord, node?: ExecutionDispatchNodePreview): void {
    const lines = [
      ...
      ...(run.terminal_outcome ? [
        "## Terminal outcome",
        `- Code: ${run.terminal_outcome.code}`,
        `- Severity: ${run.terminal_outcome.severity}`,
        `- Detail: ${run.terminal_outcome.detail}`,
        `- Changed files: ${run.terminal_outcome.changed_file_count}`,
        `- Summary present: ${run.terminal_outcome.summary_present ? "yes" : "no"}`,
        "",
      ] : []),
      "## Result",
